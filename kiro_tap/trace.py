...
from kiro_tap.trace_store import TraceStore, get_trace_store
...
class TraceWriter:
    ...
    def _update_stats(self, record: dict) -> None:
        ...
        # Kiro stores usage in kiro_usage field on the reconstructed snapshot
        if not usage and isinstance(resp_body, dict):
            kiro_usage = resp_body.get("kiro_usage")
            if isinstance(kiro_usage, dict):
                usage = kiro_usage
