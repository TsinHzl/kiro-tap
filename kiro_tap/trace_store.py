...
def resolve_db_path() -> Path:
    ...
    override = os.environ.get("KIROTAP_DB", "").strip()
    if override:
        return Path(override).expanduser().resolve()
    xdg_data = os.environ.get("XDG_DATA_HOME", "").strip()
    if xdg_data:
        base = Path(xdg_data).expanduser() / "kiro-tap"
    else:
        base = Path.home() / ".local" / "share" / "kiro-tap"
...
class TraceStore:
    ...
    def _import_legacy_session(
        ...
        *,
        ...
        with self._write_lock:
            ...
            if row is not None:
                from kiro_tap.dashboard import build_imported_session_summary
    ...
    def _refresh_summary_after_append(
        ...
        from kiro_tap.dashboard import merge_record_into_summary
...
def _manifest_entries_by_rel_path(output_dir: Path) -> dict[str, dict[str, Any]]:
    manifest_path = output_dir / ".kirotap-manifest.json"
    try:
